   ...
   <div class="row">
      <div class="col-md-12 grid-margin stretch-card">
         <div class="card">
            <div class="card-body">
               <?php
                  $applicationa= $this->session->flashdata('application');
                  $error_msg= $this->session->flashdata('error_msg');
                  
                  if($applicationa){
                  ?>
               <div class="alert alert-success" role="alert">
                  <?php echo $applicationa; ?>
               </div>
               <?php
                  }
                  if($error_msg){
               ...
               <?php
                  ...
                  ?>
               <form class="forms-sample" method="POST" action="<?php echo base_url('admin/update_application'); ?>" enctype="multipart/form-data">
                  <div class="form-group">
                     ...
                     <select class="js-example-basic-multiple w-100" id="cat_value" multiple="multiple" name="categories[]" required>
                        ...
                        <?php $filter = $this->db->get_where('filter',array('id' =>2))->row_array();?>
                        <?php 
                  ...
                  <div class="form-group">
                     ...
                     <select class="js-example-basic-multiple w-100" multiple="multiple" id="new_sub" name="sub_cat[]" required>
                        ...
                        <?php 
                           ...
                           foreach($subcategories as $sub_cat) {
                           ?>
                        <?php $subcate=explode(",", $apps->sub_cat); ?>
                        <option <?php if(in_array($sub_cat['id'],$subcate)) { ?> selected <?php } else{} ?>  value="<?php echo $sub_cat['id']; ?>"><?php echo $sub_cat['sname']; ?></option>
                        <?php } ?>
                     </select>
                  </div>
                  <div class="form-group">
                     <label for="exampleInputUsername1">Application Name</label>
                     <input type="text" name="application" value="<?php echo $apps->application; ?>"  class="form-control" id="exampleInputUsername1" placeholder="Enter Application Name">
                  </div>
                  <button name="submit" type="submit" class="btn btn-gradient-primary mr-2">Submit</button>
               </form>
            </div>
                  ...
                  <table id="order-listing" class="table">
                     ...
                     <tbody>
                        ...
                        <tr>
                           ...
                           <td>
                              <?php if(!empty($app['cat'])) { ?>
                              <?php $categories=explode(",", $app['cat']);
                           ...
                           <td>
                              <?php if(!empty($app['sub_cat'])) { ?>
                              <?php $subcategories=explode(",", $app['sub_cat']);
                           ...
                           <td>
                              <a class="btn btn-outline-danger btn-fw" href="<?php echo base_url('admin/delete_application/').$app['id'];?>"><i class="ti-trash"></i></a>
                              <a class="btn btn-outline-success btn-fw" href="<?php echo base_url('admin/edit_application/').$app['id'];?>"><i class="ti-pencil-alt"></i></a>
                           </td>
                        </tr>
                        <?php } ?> 
                     </tbody>
